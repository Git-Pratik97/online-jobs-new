...
const PostJob = () => {
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      axios
        .post(
          `http://localhost:8080/jobportal/job/postjob/${values.freelancerId}/${values.recruiterId}/${values.skillId}`,
          values
        )
        .then((resp) => {
        ...
        >
          <div className="form-group">
            ...
            {formik.errors.skillId ? (
              <div className="errors">{formik.errors.skillId}</div>
            ) : null}
          </div>
          ...
          <div className="form-group">
            ...
            {formik.errors.postedDate ? (
              <div className="errors">{formik.errors.postedDate}</div>
            ) : null}
          </div>
